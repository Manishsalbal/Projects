...
public class UserDataController {
	...
	public void registerItems(@RequestBody UserData data) {
		...
		dat.setChequerequest(null);
		dat.setChequerequeststatus(null);
		dat.setLoanrequest(null);
		dat.setLoanamount(null);
		dat.setLoanstatus(null);
		myrepo.save(dat);
...
//	}
	...
	public void betweenAccount(@PathVariable("name") String name,@RequestBody UserData Userdetails ) {
		...
		if(Userdetails.getSavingbalance() != null) {
			UserData data = myrepo.findByNames(name);
			if(data.getBalance() >= Userdetails.getSavingbalance()) {
		...
		if(Userdetails.getBalance() != null) {
			UserData data = myrepo.findByNames(name);
			if(data.getSavingbalance() >= Userdetails.getBalance()) {
				data.setBalance((data.getBalance())+(Userdetails.getBalance()));
	...
	public void setStatus(@PathVariable("name") String name, @RequestBody UserData Userdetails) {
		UserData data = myrepo.findByNames(name);
		data.setAccountstatus(Userdetails.getAccountstatus());
		myrepo.save(data);
	}
	
	
	@PutMapping("/chequerequest/{name}")
	public void setchequerequest(@PathVariable("name") String name, @RequestBody UserData Userdetails) {
		UserData data = myrepo.findByNames(name);
		data.setChequerequest(Userdetails.getChequerequest());
		myrepo.save(data);
	}
	
	@PutMapping("/loanrequest/{name}")
	public void setloanrequest(@PathVariable("name") String name, @RequestBody UserData Userdetails) {
		UserData data = myrepo.findByNames(name);
		data.setLoanrequest(Userdetails.getLoanrequest());
		data.setLoanamount(Userdetails.getLoanamount());
		myrepo.save(data);
	}
	
	@PutMapping("/loanbyadmin/{name}")
	public void setloanstatus(@PathVariable("name") String name, @RequestBody UserData Userdetails) {
		UserData data = myrepo.findByNames(name);
		data.setLoanstatus(Userdetails.getLoanstatus());
		myrepo.save(data);
	}
	
	@PutMapping("/chequebyadmin/{name}")
	public void setchequestatus(@PathVariable("name") String name, @RequestBody UserData Userdetails) {
		UserData data = myrepo.findByNames(name);
		data.setChequerequeststatus(Userdetails.getChequerequeststatus());
		myrepo.save(data);
	}
